...
class UpdateEvent extends StatefulWidget {
  EventDm eventDm;
   UpdateEvent( this.eventDm,{super.key});

  @override
  State<UpdateEvent> createState() => _AddEventState();
}

class _AddEventState extends State<UpdateEvent> {
  ...
  @override
  void initState() {
     titleControler.text = widget.eventDm.title;
    descriptionController.text = widget.eventDm.description;
      selectedCatogry = CatogryDm.fromtitle(widget.eventDm.catogryId);
  ...
  buildAppBar() => AppBar(title: Text("update event"));
      ...
      onClick: () async {
        if (UserDm.currentUser == null) {
          showMessege(
            context,
            title: "Not signed in",
            content: "You must be signed in to update events.",
            posButtonTitle: "OK",
          );
          return;
        }
        ...
        await updateEventToFirestore(eventDm);
